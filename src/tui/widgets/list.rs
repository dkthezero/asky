...
pub fn render(
    ...
    let items: Vec<ListItem> = packages
        ...
        .map(|pkg| {
            ...
            let status = install_status(config, &pkg.vault_id, &pkg.identity.name, &pkg.kind, &pkg.identity.sha10);
...
pub fn render_vaults(frame: &mut Frame, area: Rect, vaults: &[VaultEntry], selected: usize) {
    ...
    let items: Vec<ListItem> = vaults.iter().map(|v| {
        let check = if v.enabled { "[x]" } else { "[ ]" };
        ListItem::new(Line::from(format!("{} {:<20} {:<8} {}", check, v.id, v.kind, v.counts_label())))
    }).collect();
    let list = List::new(items).block(block)
        .highlight_style(Style::default().bg(Color::Blue).fg(Color::White).add_modifier(Modifier::BOLD))
        .highlight_symbol("> ");
    let mut state = ListState::default();
    if !vaults.is_empty() { state.select(Some(selected)); }
    frame.render_stateful_widget(list, area, &mut state);
}

pub fn render_providers(frame: &mut Frame, area: Rect, providers: &[ProviderEntry], selected: usize) {
    ...
    let list = List::new(items).block(block)
        ...
        .highlight_symbol("> ");
    let mut state = ListState::default();
    if !providers.is_empty() { state.select(Some(selected)); }
    frame.render_stateful_widget(list, area, &mut state);
}

fn install_status(config: &ConfigFile, vault_id: &str, name: &str, kind: &AssetKind, current_hash: &str) -> &'static str {
    match kind {
        AssetKind::Skill => {
            if let Some(installed_hash) = config.installed_skill_hash(vault_id, name) {
                if installed_hash != current_hash { "[!]" } else { "[✓]" }
            } else { "[ ]" }
        }
