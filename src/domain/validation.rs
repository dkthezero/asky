...
pub fn validate_git_repo(repo: &str) -> Result<()> {
    ...
    if parts.len() != 2 {
        bail!(
            "Invalid git repository format: '{}'. Expected 'owner/repo'.",
            repo
        );
    }
...
pub fn validate_git_ref(r#ref: &str) -> Result<()> {
    ...
    if r#ref.chars().any(|c| c.is_control()) {
        bail!(
            "Invalid git ref: '{}'. Cannot contain control characters.",
            r#ref
        );
    }
    Ok(())
...
pub fn validate_git_path(path: &str) -> Result<()> {
    if path.starts_with('-') {
        bail!(
            "Invalid git subfolder path: '{}'. Cannot start with '-'.",
            path
        );
    }
    if path.contains("..") {
        bail!(
            "Invalid git subfolder path: '{}'. Traversal with '..' is not allowed.",
            path
        );
    }
    Ok(())
